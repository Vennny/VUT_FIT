...
**/
	...
	if(!(*RootPtr)){		//strom je prazdny

		*RootPtr = malloc(sizeof(struct tBTNode));	//maloc a osetreni
		...
		tBTNodePtr search = NULL;
		tBTNodePtr tmp = *RootPtr;

		while(tmp){

			search = tmp;		//pamatovani mista

			if(tmp->Cont < Content)	//vkladany content je vetsi, posun vpravo
				tmp = tmp->RPtr;

			else if (tmp->Cont > Content)	//vkladany content je mensi, posun vlevo
				tmp = tmp->LPtr;
		...
		tBTNodePtr new = malloc(sizeof (struct tBTNode));	//vytvoreni noveho uzlu
		if (!new)
			return;
...
**/
	while(ptr){			
		...
		SPushP(Stack, ptr);		//ulozeni do zasobniku
		ptr = ptr->LPtr;		//jdeme na levy
	}
}
...
**/
	...
	Leftmost_Preorder(RootPtr, &stack);	//leva vetev do zasobniku

	tBTNodePtr tmp;
	while (!SEmptyP(&stack)){			//pruchod zasobnikem
...
**/
	...
	while (ptr) { 					//pruchod
		SPushP(Stack, ptr);			//push ukazatele
...
**/
	...
	Leftmost_Inorder(RootPtr, &stack);	//leva vetev do zasobniku
...
**/
	...
	tStackB stackB;		//deklarace zasobniku hodnot v bool
	tStackP stackP;		//deklarace zasobniku ukazatelu 

	SInitB(&stackB);	//inicializace zasobniku hodnot v bool
	SInitP(&stackP);	//inicializace zasobniku ukazatelu 


	Leftmost_Postorder(RootPtr, &stackP, &stackB);	//leva vetev do zasobniku

	while( !SEmptyP(&stackP) ){			//pruchod zasobnikem

		tBTNodePtr tmp = STopPopP(&stackP);		//ulozim vrsek zasobniku
		SPushP(&stackP, tmp);					//vracim zpet vrsek zasobniku

		if(STopPopB(&stackB)){			//v uzlu jeste nebyl
			SPushB(&stackB, false);			
...
**/
	...
	tStackP stackP;		//deklarace zasobniku 
	SInitP(&stackP);		//inicializace zasobniku

	while( (*RootPtr) || !SEmptyP(&stackP)){	//dokud ptr existuje nebo zasobnik neni prazdny
		
		if(!(*RootPtr))		// pokud je ukazatel null, ulozi obsah zasobniku do korene
			*RootPtr = STopPopP(&stackP);

		if( (*RootPtr)->RPtr) //jestli ma pravou vetev, ulozi ji na zasobnik
