------------------------------------------------
-- IDS projekt
-- Ondřej Studnička, Václav Trampeška
-- 25. 4. 2019
------------------------------------------------
...
-- trigger na zjištění jestli je sál již obsazen
...
-- procedura na výpis percentuálního zastoupení různých věkových skupin zákazníkù v databázi
...
-- procedura na zjisteni celkových výdělků jednotlivých filmů 
...
-- procedura na vypsání tržeb jednotlivých filmů
EXEC celkova_trzba_jednotlivych_filmu();

-- použití explain plan pro zjištění ceny dotazu
EXPLAIN PLAN FOR 
    SELECT promitani.id_promitani AS ID, promitani.datum AS DATUM, TO_CHAR(promitani.cas, 'hh24:mi') AS CAS, AVG(listek.cena) AS prumerny_cena_na_osobu
    FROM promitani, listek
    WHERE listek.id_promitani = promitani.id_promitani
...
--opìtovné volání pro zjištění ceny stejného dotazu (cena se snížila)
EXPLAIN PLAN FOR 
    SELECT promitani.id_promitani AS ID, promitani.datum AS DATUM, TO_CHAR(promitani.cas, 'hh24:mi') AS CAS, AVG(listek.cena) AS prumerny_cena_na_osobu
    FROM promitani, listek
    WHERE listek.id_promitani = promitani.id_promitani
